...
class SelectFiles(Gtk.VBox):
    ...
    def __init__(self, file_list, *args, **kwargs):
        ...
        # Choose Files Box
        self.box_choose_files = Gtk.HBox(
            spacing=6, valign=Gtk.Align.CENTER, halign=Gtk.Align.CENTER)
        self.pack_start(self.box_choose_files, True, True, 0)

        # Choose Files Label
        self.label_choose_files = Gtk.Label(label='Choose files to rename')
        self.box_choose_files.pack_start(
            self.label_choose_files, True, True, 0)

        # Choose Files Button # FileChooserButton
        self.icon_open = Gtk.Image(icon_name='document-open-symbolic')
        self.button_choose_files = Gtk.Button(
            image=self.icon_open, always_show_image=True)
        self.button_choose_files.connect('clicked', self.__on_select_files)
        self.box_choose_files.pack_start(
            self.button_choose_files, True, True, 0)
